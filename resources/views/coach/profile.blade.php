        ...
        <section class="rounded-2xl border border-slate-800 bg-slate-900/50 p-6 sm:p-8">
            <form action="{{ route('coach.profile.update') }}" method="POST" class="space-y-4">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <div class="space-y-1">
                        <label for="mobile_number" class="block text-xs font-medium text-slate-400">Mobile Number</label>
                        <input type="text" id="mobile_number" name="mobile_number"
                            value="{{ old('mobile_number', auth()->user()->mobile_number) }}"
                            class="block w-full rounded-md border {{ $errors->has('mobile_number') ? 'border-red-500' : 'border-slate-700' }} bg-slate-950/60 px-3 py-2 text-sm text-slate-50 placeholder-slate-500 focus:border-emerald-400 focus:outline-none focus:ring-1 focus:ring-emerald-400"
